...
class productClass:
    def __init__(self, root):
        ...
        # === Variables ===
        self.var_searchby = StringVar(value="Category")
        self.var_searchtxt = StringVar()
        ...
        # === Product Form ===
        lbl_category = Label(self.root, text="Category", font=("goudy old style", 15), bg="white")
        lbl_category.place(x=30, y=60)
        self.cmb_category = ttk.Combobox(self.root, textvariable=self.var_category, font=("goudy old style", 13), state='readonly', justify=CENTER)
        ...
        self.ProductTable = ttk.Treeview(product_frame, columns=("category", "name", "price", "qty", "status"),
    ...
    def fetch_categories(self):
        try:
            data = self.category_collection.find()
    ...
    def show(self):
        ...
        for doc in self.collection.find():
            self.ProductTable.insert("", END, values=(
                doc.get("category", ""),
                doc.get("name", ""),
                doc.get("price", ""),
                doc.get("qty", ""),
    ...
    def get_data(self, ev):
        ...
        data = self.ProductTable.item(selected)["values"]
        if data:
            self.var_category.set(data[0])
            self.var_name.set(data[1])
            self.var_price.set(data[2])
            self.var_qty.set(data[3])
            self.var_status.set(data[4])

    def update(self):
        try:
            query = {"name": self.var_name.get()}
            update_data = {
                "$set": {
                    "category": self.var_category.get(),
                    "price": self.var_price.get(),
