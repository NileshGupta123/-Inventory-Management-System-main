...
#         tk.Label(frame1, text="Customer Name").grid(row=0, column=0, padx=5)
#         self.name_var = tk.StringVar()
#         tk.Entry(frame1, textvariable=self.name_var, bg="lightyellow").grid(row=0, column=1, padx=5)

#         tk.Label(frame1, text="Contact No").grid(row=0, column=2, padx=5)
#         self.contact_var = tk.StringVar()
#         tk.Entry(frame1, textvariable=self.contact_var, bg="lightyellow").grid(row=0, column=3, padx=5)

#         # Product Selection
#         tk.Label(frame1, text="Select Product").grid(row=1, column=0, padx=5)
#         self.product_var = tk.StringVar()
#         self.product_cb = ttk.Combobox(frame1, textvariable=self.product_var, width=30)
#         self.product_cb.grid(row=1, column=1, padx=5)

#         tk.Label(frame1, text="Quantity").grid(row=1, column=2, padx=5)
#         self.qty_var = tk.IntVar(value=1)
#         tk.Entry(frame1, textvariable=self.qty_var, width=10).grid(row=1, column=3, padx=5)

#         tk.Button(frame1, text="Add to Bill", command=self.add_to_bill).grid(row=1, column=4, padx=10)

#         # Product Table
#         frame2 = tk.Frame(self.root)
#         frame2.pack(side="left", fill="both", expand=True)

#         self.tree = ttk.Treeview(frame2, columns=("name", "price", "quantity"), show='headings')
...
#         # Bill Area
#         frame3 = tk.Frame(self.root, bg="orange", bd=2)
#         frame3.pack(side="right", fill="both", expand=True)

#         tk.Label(frame3, text="Customer Bill", bg="orange", font=("Arial", 14)).pack()
#         self.bill_text = tk.Text(frame3, bg="lightyellow")
...
#         tk.Button(frame4, text="Generate Bill", bg="green", fg="white", command=self.generate_bill).pack(pady=5)

#     def load_products(self):
#         products = list(self.products_collection.find())
#         self.product_cb['values'] = [f"{p['name']} (Qty: {p['quantity']})" for p in products]

#     def get_product_details(self, name):
#         return self.products_collection.find_one({"name": name})
...
#         price = product["price"]
#         self.tree.insert('', 'end', values=(name, price, qty))
#         self.cart.append({"name": name, "price": price, "quantity": qty})
...
#             product = self.get_product_details(item['name'])
#             if not product:
#                 messagebox.showerror("Error", f"Product '{item['name']}' not found in DB")
#                 return
...
#         self.billing_collection.insert_one(bill_data)
#         self.sales_collection.insert_one(bill_data)

#         self.bill_text.delete(1.0, tk.END)
#         self.bill_text.insert(tk.END, f"Invoice No: {invoice_no}\n")
#         self.bill_text.insert(tk.END, f"Customer Name: {customer_name}\n")
#         self.bill_text.insert(tk.END, f"Contact No: {contact}\n")
#         self.bill_text.insert(tk.END, f"{'-'*30}\n")
#         for item in items:
#             self.bill_text.insert(tk.END, f"{item['name']} - {item['quantity']} x {item['price']}\n")
#         self.bill_text.insert(tk.END, f"{'-'*30}\n")
#         self.bill_text.insert(tk.END, f"Total: Rs. {total}\n")

#         messagebox.showinfo("Success", f"Bill generated with Invoice No: {invoice_no}")
#         self.cart.clear()
...
class BillingApp:
    def __init__(self, root):
        ...
        # MongoDB Connection
        self.client = pymongo.MongoClient("mongodb://localhost:27017/")
        self.db = self.client["billing_system"]  # ✅ Must match sales.py
        self.billing_collection = self.db["billing"]
        self.products_collection = self.db["products"]
        self.sales_collection = self.db["sales"]
